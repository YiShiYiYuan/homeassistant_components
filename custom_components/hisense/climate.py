...
class HisenseACClimate(ClimateEntity):
    def __init__(self, api):
        ...
        self._attr_supported_features = (
            ClimateEntityFeature.TURN_ON | ClimateEntityFeature.TURN_OFF | ClimateEntityFeature.TARGET_TEMPERATURE | ClimateEntityFeature.FAN_MODE | ClimateEntityFeature.SWING_MODE
        )
        self._enable_turn_on_off_backwards_compatibility = False
        self._attr_fan_modes = [FAN_AUTO, FAN_DIFFUSE, FAN_LOW, FAN_MEDIUM, FAN_HIGH]
        self._attr_hvac_modes = [HVACMode.COOL, HVACMode.HEAT, HVACMode.DRY, HVACMode.FAN_ONLY, HVACMode.OFF]
        self._attr_swing_modes = [SWING_ON, SWING_OFF, SWING_HORIZONTAL, SWING_VERTICAL]
        ...
        self._fan_mode_lookup = {0: FAN_AUTO, 1: FAN_DIFFUSE, 2: FAN_LOW, 3: FAN_MEDIUM, 4: FAN_HIGH}
        self._fan_mode_to_id = {FAN_AUTO: 0, FAN_DIFFUSE: 1, FAN_LOW: 2, FAN_MEDIUM: 3, FAN_HIGH: 4}
        self._swing_mode_lookup = {0: SWING_OFF, 1: SWING_ON, 2: SWING_HORIZONTAL, 3: SWING_VERTICAL}
        self._swing_mode_to_id = {SWING_OFF: 0, SWING_ON: 1, SWING_HORIZONTAL: 2, SWING_VERTICAL: 3}
    ...
    async def async_update(self):
        ...
        if self._attr_is_on:
            self._attr_hvac_mode = self._hvac_mode_lookup[status.get("hvac_mode_id", 4)]
        else:
            self._attr_hvac_mode = HVACMode.OFF
        self._attr_fan_mode = self._fan_mode_lookup[status.get("fan_mode_id", 0)]
        self._attr_swing_mode = self._swing_mode_lookup[status.get("swing_mode_id", 0)]
