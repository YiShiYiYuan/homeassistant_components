...
class HiSenseApi:
    ...
    async def send_logic_command(self, id: int, param: int):
        ...
        command_data["cmdList"] = [{"cmdId": id, "cmdOrder": 0, "cmdParm": param, "delayTime": 0}]
    ...
    async def refresh(self):
        ...
        try:
            async with self.session.post(self.refresh_url, headers=self.refresh_headers, data=refresh_data) as response:
                result = await response.json()
...
if __name__ == "__main__":
    ...
    async def main():
        wifi_id = "86100c00fffe005000000040e2114f22"
        device_id = "1kk0500p9n0000ac7pe0070"
        appkey = "1537849890"
        refresh_token = "9aPF8HAAF04D9RAxU2VQrx1W2wXsE-XkR4npjsHrbjlxUGHBeVHn42eYWb8QQi8Y9UiMwCyWrrFl2alxyA6a3fiFnuiaaSy1aYPBYJEsJ81mWShEnJwXFTnmFSOTiu1749QWa4ja2ws6zQvfQfmQMfPmrhgNKF69tjCjhHXDk1knRyspnX1TA8TSed12B8Kncv8d7vwGWw"

        session = aiohttp.ClientSession()
        hsa = HiSenseApi(wifi_id, device_id, appkey, refresh_token, session)

        id = 41
        param = 1
        # await hsa.send_logic_command(id, param)
        # await hsa.refresh()
        await hsa.check_status()
        print(hsa.get_status())
        await session.close()
