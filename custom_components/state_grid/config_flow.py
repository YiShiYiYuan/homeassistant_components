...
class StateGridConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_scan_login(self, user_input=None):
        ...
        if result["errcode"] != 0:
            return self.async_abort(reason="get_qr_code_error", description_placeholders={"errmsg": result["errmsg"]})

        return self.async_show_form(
            step_id="check_qr_code", description_placeholders={"qr_image": '<img style="width: 200px;" src="data:image/png;base64,' + result["data"] + '"/>'}
        )
    ...
    async def async_step_check_qr_code(self, user_input=None):
        ...
        if result["errcode"] != 0:
            return self.async_abort(reason="check_qr_code_error", description_placeholders={"errmsg": result["errmsg"]})
    ...
    async def async_step_code_login(self, user_input=None):
        ...
        data_schema = {vol.Required("phone"): selector({"text": {"type": "number"}})}

        return self.async_show_form(step_id="code_login", data_schema=vol.Schema(data_schema), errors=errors)
    ...
    async def async_step_verfiy_code(self, user_input=None):
        ...
        data_schema = {vol.Required("code"): selector({"text": {"type": "number"}})}

        return self.async_show_form(step_id="verfiy_code", data_schema=vol.Schema(data_schema), errors=errors)
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_debug(self, user_input=None):
        ...
        data_schema = {
            ...
            vol.Required("is_debug", default=self.data_client.is_debug): selector({"boolean": {}}),
        }

        return self.async_show_form(step_id="debug", data_schema=vol.Schema(data_schema))
